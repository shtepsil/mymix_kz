...
(function ($) {
    ...
    function fnCreateDateRangeInput() {
        ...
        var sFromId = sTableId + 'range_from_' + i;
        var from = $('<input type="text" class="input-sm form-control" id="' + sFromId + '" rel="' + i + '"/>');
        from.datepicker();
        th.append(from);
        th.append($('<span class="input-group-addon">-</span>'));
        //th.append(_fnRangeLabelPart(1));
        var sToId = sTableId + 'range_to_' + i;
        var to = $('<input type="text" class="input-sm form-control" id="' + sToId + '" rel="' + i + '"/>');
        th.append(to);
        //th.append(_fnRangeLabelPart(2));
        th.wrapInner('<div class="input-daterange input-group" style="width: 208px;" />');
        to.datepicker();
        var index = i;
        aiCustomSearch_Indexes.push(i);

        $('#' + sFromId + ',#' + sToId, th).on('changeDate', function () {
            //oTable.fnDraw();
            var dStartDate = from.datepicker("getDate");
            var dEndDate = to.datepicker("getDate");
            oTable.fnFilter(dStartDate.valueOf() + '-' + dEndDate.valueOf(), index);
        });
    }
    ...
    function fnCreateSelect(aData) {
        var index = i;
        var r = '<select class="search_init select_filter form-control"><option value="" class="search_init"></option>', j, iLen = aData.length;
        $.each(aData, function (val, title) {
    ...
    $.fn.columnFilter = function (options) {
        ...
        return this.each(function () {
            ...
            var nTHead = oTable.fnSettings().nTHead;
            var nTr = $(nTHead).children('tr')[0];
            var content = $('<tr></tr>');
            $("th", nTHead).each(function (index) {
                i = index;
                var aoColumn = {
                    type: null,
                    bRegex: false,
