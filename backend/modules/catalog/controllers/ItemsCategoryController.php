...
use backend\modules\catalog\models\ItemsCategory;
...
class ItemsCategoryController extends AdminController
{
    public function init()
    {
        $this->model = new ItemsCategory();
        $controller_name = Inflector::camel2id($this->id);
        $this->url = [
            'back' => ["$controller_name/index"],
            'control' => ["$controller_name/control"]
        ];
        $this->view->title = 'ItemsCategory';
        $this->breadcrumb[] = [
