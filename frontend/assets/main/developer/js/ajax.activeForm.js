...
(function ($) {
    ...
    var methods = {
        init: function (attributes, options) {
            return this.each(function () {
                ...
                var messages = {},
                    submitting = false;
                console.log('->developer/js/ajax.activeForm.js');
                ...
                $form.ajaxForm(
                    {
                        ...
                        success: function (data_return) {
                            ...
                            if (typeof data_return != 'undefined') {
                                ...
                                $.growl.error({title: 'Ошибка', message: "Произошла ошибка на стороне сервера5!", duration: 5000});
                            }
                        },
                        error: function (data) {
                            $('.res').html('Error:<br>'+JSON.stringify(data));
                            $('#loader').hide();
                            $.growl.error({title: 'Ошибка', message: "Произошла ошибка на стороне сервера4!", duration: 5000});
                        }
                    }
                );
            });
            ...
            if (hasError) {
                ...
                if ($element.attr('type') != 'hidden') {
                    if (!$($element).data('tooltipster-ns')) {
                        ...
                        $($element).tooltipster('content', messages[attribute.id][0]);
                ...
                if ($element.attr('type') != 'hidden' && $($element).data('tooltipster-ns')) {
    ...
    var updateSummary = function ($form, messages) {
        ...
        if ($summary.length && messages) {
            ...
            $.growl.error({title: 'Ошибка', message: 'Вы незаполнили обязательные поля', duration: 5000});
        }
