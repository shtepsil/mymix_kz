...
use backend\modules\catalog\models\ItemQuestions;
...
class SendQuestionItem extends Model
{
    public $body;
    public $username;
    public $email;
...
//        $record->user_id = $user->id;
        ...
        $mailer = \Yii::$app->mailer->compose(['html' => 'message-send'], ['item' => $this])
            ...
            ->setTo($send_mails)
            ->setSubject('Новый вопрос ' . \Yii::$app->params['siteName'] . '.kz');
        \Yii::$app->language = $current_lang;
        if ($record->save(false)) {
            $mailer->send();
            $message = Json::encode(\Yii::t('main', 'Ваш вопрос отправлен и проверяеться модератором.'));
            $result['js'] = <<<JS
$('.overlayWinmod').winmod('close', 'goods-question');
$('.window__description','[data-winmod="thanks"]').html({$message})
$('.overlayWinmod').winmod('win_open', 'goods-question');
\$form.resetForm();
JS;
        } else {
